...
int32_t CProtocolClientCfg::packageCapResultInfo(int8_t * buff, uint32_t buffLen, IlligalProcResult &res, int sendPic)
{
	if (NULL == buff || buffLen < 0) {
	...
	*p = res.camNum.length();// 球机编号长度
	p++;
	strncpy(p, res.camNum.c_str(), res.camNum.length());
	p += res.camNum.length();

	*((uint32_t *)(p)) = res.plateVariety;
	p += sizeof(uint32_t);

	*p = res.plateColor;
	p++;

	*p = res.plateNum.length();// 车牌号长度
	p++;
	strncpy(p, res.plateNum.c_str(), res.plateNum.length());
	p += res.plateNum.length();
	
	*p = res.vedioName.length();// 录像名长度
	p++;
	strncpy(p, res.vedioName.c_str(), res.vedioName.length());
	p += res.vedioName.length();
	...
	*((uint32_t *)(p)) = res.missionNum;
	p += sizeof(uint32_t);
	//printf();

	*p = res.capTime.length();// 抓拍日期长度
	p++;
	strncpy(p, res.capTime.c_str(), res.capTime.length());
	p += res.capTime.length();

	if(sendPic){

		for(int i=0; i<res.picNum; i++){

			*p = res.picName[i].length();// 录像名长度
			p++;
			strncpy(p, res.picName[i].c_str(), res.picName[i].length());
			p += res.picName[i].length();
			
			int fd = open(res.picName[i].c_str(), O_RDONLY);	
			printf("open fileName %s, file len = %d\r\n", res.picName[i].c_str());
			if(fd > 0){

				unsigned long PicSize = get_file_size(res.picName[i].c_str());
				int rSize = read(fd, p, PicSize);
				printf("read size = %d\r\n", PicSize);
				p += PicSize;
				close(fd);
			}else{
