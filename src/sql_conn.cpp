			...
			// printf("[sql callback]: capDevs\n");
			 if("missionID" == sColName){
				...
				if(-1 == MissionAddCapDev(&G_capdev)){
					...
					printf("G_missionUnstart G_missionUnstart.totalCapDev = %d \r\n", G_missionUnstart.totalCapDev);
				}
			 }
...
int8_t CSQL_conn::update(missionGroupChgReq &req)
{
	...
	if(0 == req.type){
		...
		sprintf(sql, "UPDATE capDevs SET missionID=%d "  \
			 "WHERE devNum='%s'; " ,\
			 req.dts_num, req.num.c_str());
	 ...
	 rc = sqlite3_exec(handle, sql, callback, 0, &zErrMsg);
	 if( rc != SQLITE_OK ){
		fprintf(stderr, "[UPDATE]:SQL UPDATE missionGroupChgReq error: %s\n", zErrMsg);
		sqlite3_free(zErrMsg);
	 }else{
		fprintf(stdout, "UPDATE  missionGroupChgReq successfully\n");
	 }
